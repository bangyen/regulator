#!/usr/bin/env python3
"""
Enhanced experiment using collusive agents to demonstrate leniency program effects.

This script uses collusive agents that actively try to coordinate prices,
making it easier to observe the effects of the leniency program on breaking
collusion and improving market outcomes.
"""
...
def run_collusive_episode_with_leniency(
    env: CartelEnv,
    regulator: Regulator,
    agents: List[Any],
    max_steps: int = 100,
    ...
    """
    Run a single episode with collusive agents and leniency program.
    """
    if seed is not None:
        np.random.seed(seed)
        ...
        for i, agent in enumerate(agents):
            if hasattr(agent, "evaluate_whistleblow_opportunity"):
                ...
                if detection_results["parallel_violation"]:
                    collusion_prob += 0.6  # Higher probability for parallel pricing
                if detection_results["structural_break_violation"]:
                    collusion_prob += 0.4

                # Also consider price level as evidence of collusion
                market_price = np.mean(actions_array)
                if market_price > 25.0:  # High prices suggest collusion
                    collusion_prob += 0.3

                if collusion_prob > 0.2:  # Lower threshold for whistleblowing
...
def run_collusive_episode_without_leniency(
    env: CartelEnv,
    regulator: Regulator,
    agents: List[Any],
    max_steps: int = 100,
    ...
    """
    Run a single episode with collusive agents but without leniency program.
    """
    if seed is not None:
        np.random.seed(seed)
        env.np_random = np.random.default_rng(seed)
...
def run_collusive_leniency_experiment(
    n_episodes: int = 30,
    n_firms: int = 3,
    max_steps: int = 100,
    leniency_reduction: float = 0.6,
    ...
    """
    Run the collusive leniency experiment.
    """
    ...
    env = CartelEnv(
        ...
        demand_slope=-1.0,
        shock_std=3.0,  # Lower shock for more stable collusion
        price_min=1.0,
        price_max=100.0,
        seed=seed,
    )
    ...
    regulator_with_leniency = Regulator(
        parallel_threshold=3.0,  # Slightly higher threshold
        parallel_steps=2,  # Fewer steps needed
    ...
    leniency_program = LeniencyProgram(
        leniency_reduction=leniency_reduction,
        evidence_threshold=0.5,  # Lower threshold
        audit_probability=0.15,  # Higher audit probability
        seed=seed,
    )
    ...
    agents_with_leniency = [
        WhistleblowerTitForTatAgent(
            i,
            leniency_program,
            whistleblow_threshold=5.0,  # Lower threshold
    ...
    agents_without_leniency = [
        CollusiveAgent(i, collusive_price=collusive_price, seed=seed)
        for i in range(n_firms)
    ]

    # Run experiments
    print(
        f"Running collusive leniency experiment with {n_episodes} episodes per condition..."
    )
    ...
    for episode in range(n_episodes):
        ...
        episode_data = run_collusive_episode_with_leniency(
    ...
    for episode in range(n_episodes):
        ...
        episode_data = run_collusive_episode_without_leniency(
            env,
    ...
    results_file = output_path / f"collusive_leniency_experiment_{timestamp}.json"
    ...
    # Check 4: Prices decrease (better for consumers)
    price_change = differences["avg_price_percent_change"]
    if price_change < 0:
        print(
            f"  ✓ Prices decreased by {abs(price_change):.1f}% (better for consumers)"
        )
    else:
        print(f"  ✗ Prices increased by {price_change:.1f}%")
...
def main() -> Dict[str, Any]:
    """Main function to run the collusive leniency experiment."""
    parser = argparse.ArgumentParser(description="Run collusive leniency experiment")
    parser.add_argument(
        "--episodes", type=int, default=30, help="Number of episodes per condition"
    )
    ...
    parser.add_argument(
        "--fine-amount", type=float, default=100.0, help="Base fine amount"
    )
    parser.add_argument(
        "--collusive-price", type=float, default=30.0, help="Target collusive price"
    )
    parser.add_argument(
    ...
    results = run_collusive_leniency_experiment(
        ...
        collusive_price=args.collusive_price,
