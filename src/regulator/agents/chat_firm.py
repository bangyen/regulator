...
class CollusiveChatAgent(ChatFirmAgent):
    ...
    def choose_price(
        ...
        base_price = float(self.np_random.uniform(price_min, price_max))
        # Add a bias towards higher prices
        bias = (price_max - base_price) * 0.3
        return float(min(price_max, base_price + bias))
...
class CompetitiveChatAgent(ChatFirmAgent):
    ...
    def choose_price(
        ...
        bias = (base_price - price_min) * 0.2
        return float(max(price_min, base_price - bias))
