...
class AdaptiveAgent(BaseAgent):
    ...
    def choose_price(
        ...
        n_firms = params.get("n_firms", getattr(env, "n_firms", len(observation)))
    ...
    def _explore_price(
        ...
        if strategy == "aggressive":
            ...
            base_price = (
                getattr(env, "marginal_cost", 10.0) * 1.2
            ...
            base_price = (
                getattr(env, "demand_intercept", 100.0)
                / abs(getattr(env, "demand_slope", -1.0))
                * 0.8
            )  # High price
    ...
    def _exploit_price(
        ...
        if hasattr(env, "current_demand_shock"):
            demand_shock = getattr(env, "current_demand_shock", 0.0)
    ...
    def _apply_risk_aversion(
        ...
        if risk_factor > 0.5:  # High risk
            ...
            safe_price = getattr(env, "marginal_cost", 10.0) * 1.5
